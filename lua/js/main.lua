...
function main.toggle(scope)
    if state.get_enabled(state) then
        log.debug(scope, "js is now disabled!")

        return main.disable(scope)
    end

    log.debug(scope, "js is now enabled!")
...
function main.enable(scope)
    if state.get_enabled(state) then
        log.debug(scope, "js is already enabled")
    ...
    -- saves the state globally to `_G.Js.state`
    state.save(state)
end
...
function main.disable(scope)
    if not state.get_enabled(state) then
        log.debug(scope, "js is already disabled")
